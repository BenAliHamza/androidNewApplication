...
/**
 * Adapter for displaying a list of PatientIndicator items.
 *
 * NOTE:
 *  - Does NOT call any non-existing methods (no getDisplayValue()).
 *  - Uses layout: R.layout.item_patient_indicator
 *  - Uses TextView IDs:
 *      - text_indicator_name_value
 *      - text_indicator_measured_at
 *      - text_indicator_note
 */
public class PatientIndicatorsAdapter
        extends ListAdapter<PatientIndicator, PatientIndicatorsAdapter.IndicatorViewHolder> {

    public PatientIndicatorsAdapter() {
        super(DIFF_CALLBACK);
    }
    ...
    private static final DiffUtil.ItemCallback<PatientIndicator> DIFF_CALLBACK =
            new DiffUtil.ItemCallback<PatientIndicator>() {
                ...
                public boolean areContentsTheSame(@NonNull PatientIndicator oldItem,
                                                  @NonNull PatientIndicator newItem) {
                    // For now, treat same-id as same-content. Good enough for simple list.
                    Long oldId = oldItem.getId();
                    Long newId = newItem.getId();
                    if (oldId == null || newId == null) return false;
                    return oldId.equals(newId);
    ...
    static class IndicatorViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(@Nullable PatientIndicator indicator) {
            ...
            // Measured at (raw ISO string for now)
